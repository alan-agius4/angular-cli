    ...
    for (const [name, target] of allWorkspaceTargets(workspace)) {
      ...
      switch (target.builder) {
        ...
        case Builders.Karma:
          ...
          break;
        default:
          context.logger.warn(tags.stripIndents`Cannot update "${name}" target configuration as it's using "${target}" which a 3rd party builder.
          This target configuration will require manual review.`);
